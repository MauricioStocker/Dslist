...
public class GameController {
    ...
    public String showGamePage(Model model) {
        List<GameMinDTO> games = gameService.findAll();  // Busca todos os jogos
        model.addAttribute("games", games);  // Adiciona os jogos ao modelo
        return "index";  // Retorna a view para exibir os jogos
    }
...
public ResponseEntity<Game> addGame(@RequestParam("game") String gameJson,
    ...
    Game savedGame = gameService.saveGameWithImages(gameDto, imageFiles);  // Agora o método também recebe as imagens
    ...
    public ResponseEntity<Image> addImageToGame(@PathVariable Long id, @RequestParam("imageFile") MultipartFile imageFile) throws IOException {
        // Chama o ImageService para salvar a imagem para o jogo com ID específico
        Image savedImage = imageService.saveImage(id, imageFile, false);  // O 'false' pode ser um indicador de que é a imagem não principal
